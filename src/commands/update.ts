...
function parseYesNo(v: unknown, flagName: string): boolean {
...
function uniqAppend(arr: string[], items: string[]): boolean {
    let changed = false;
...
function removeExact(arr: string[], items: string[]): boolean {
...
export const updateCommand: CommandHandler = (flags) => {
    if (flags.get("--help")) {
    ...
    try {
        ...
        if (summary) { r.summary = summary; changed = true; }

        const project = flagString(flags, "--project");
        if (project) { r.project = project; changed = true; }

        const tone = flagString(flags, "--tone");
        if (tone) { r.style.tone = tone; changed = true; }

        const format = flagString(flags, "--format");
        if (format) { r.style.format = format; changed = true; }
        ...
        if (addGoals.length) changed = uniqAppend(r.goals, addGoals) || changed;

        const removeGoals = flagStrings(flags, "--remove-goal");
        if (removeGoals.length) changed = removeExact(r.goals, removeGoals) || changed;

        // Constraints
        const addConstraints = flagStrings(flags, "--add-constraint");
        if (addConstraints.length) changed = uniqAppend(r.constraints, addConstraints) || changed;

        const removeConstraints = flagStrings(flags, "--remove-constraint");
        if (removeConstraints.length) changed = removeExact(r.constraints, removeConstraints) || changed;

        // Notes
        const addNotes = flagStrings(flags, "--add-note");
        if (addNotes.length) changed = uniqAppend(r.last_state.notes, addNotes) || changed;
        ...
        if (code === "PARSE_ERROR") return 3;
        if (code === "READ_ERROR") return 4;
